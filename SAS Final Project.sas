...
Libname Final "/home/u62726837/SAS Final Project";
data d0531;
set Final.cd0531;
PS = Province_State;
C_0531 = confirmed; *rename columns;
D_0531 = Deaths;
R_0531 = Recovered;
A_0531 = Active;
...
data FPQ2;
set FP;
IC0131 = C_0601-C_0531;  *get the new confirmed case for each day;
...
class REGION;
output out=avg_0601confirmed_region (drop=_type_ _freq_)  mean(C_0601)=TOTAL_0601_CF;
run;
...
class REGION;
output out=avg_0607confirmed_region (drop=_type_ _freq_)  mean(C_0607)=TOTAL_0607_CF;
run;
...
*According to the output data, the state which has the highest number of confirmed cases on Jun.1 is New York, 
 the state which has the highest number of confirmed cases on Jun.1 is American Samoa.;
data d0607;
set Final.cd0607;
PS = Province_State;
C_0607 = confirmed;
...
*According to the output data, the state which has the highest number of confirmed cases on Jun.7 is New York, 
 the state which has the highest number of confirmed cases on Jun.7 is American Samoa.;
...
*According to the output data, the state which has the highest number of confirmed cases in the study period is New York, 
 the state which has the highest number of confirmed cases in the study period is American Samoa.;
